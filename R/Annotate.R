...
#  mcols(gtf_gr) <- mcols(gtf_gr)[c("type","gene_id","gene_type","gene_name")]
  ...
  idx_to_annotate <- GenomicAlignments::queryHits(all_hits)
  ...
  if (transcriptDetails)
  {
    ...
    idx_to_annotate_3UTR <- GenomicAlignments::queryHits(all_UTR_3_hits)
    ...
    idx_to_annotate_5UTR <- GenomicAlignments::queryHits(all_UTR_5_hits)
    ...
    idx_to_annotate_introns <- GenomicAlignments::queryHits(all_intron_hits)
    ...
    idx_to_annotate_exons <- GenomicAlignments::queryHits(all_exon_hits)
    df$exon <- ""
    ...
    idx_to_annotate_CDS <- GenomicAlignments::queryHits(all_CDS_hits)
    df$CDS <- ""

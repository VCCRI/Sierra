...
If you have multiple BAM files the count matrices can be aggregated together using ``AggregatePeakCounts``. This function will append an experiment-specific label to the cell barcodes, by default a number according to the number of experiments, though here we define the labels (using the exp.labels variable) according to experimental condition. It's important to note that downstream if you want to combine Sierra peak counting with prior clustering the barcode labels will need to match what you currently have. If you have processed your data using the CellRanger aggr function, by inputting you data-files to ``AggregatePeakCounts`` in the same order, and leaving exp.labels blank, the barcodes should match.
```
...
results.file <- paste0(extdata_path,"/Cycling_vs_resting_fibro_UTR_length_res.tsv")
res.table <- read.table(results.file, sep = "\t", header = TRUE, 

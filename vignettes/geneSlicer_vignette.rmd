...
```{r}
scpolya::find_polyA(output.file, reference.file, bamfile, junctions.file) 
```
...
merged.peak.table = scpolya::do_peak_merging(peak.dataset.table, output.file = output.file, ncores = 4)
```
...
peak.counts = scpolya::readMEX(mm.file = paste(extdata_path,"matrix.mtx",sep="/"),
...
peaks.seurat = scpolya::polya_seurat_from_gene_object(peak.data = peak.counts, 
...
```{r}
res.table = scpolya::apply_DEXSeq_test(peaks.seurat, population.1 = "F-SL", population.2 = "EC1",
...
```{r}
scpolya::do_arrow_plot(peaks.seurat, "Cxcl12", population.ids = c("F-SL", "EC1"))

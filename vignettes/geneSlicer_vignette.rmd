---
title: "Sierra: Extracting transcript isoform use from single cell RNAseq"
author: 
  - name: Ralph Patrick
...
The following demonstrates how to download SIERRA, install the required databases, and then visualise canonical and backsplice junctions counts of the gene Slc8a1.  


STEP1: Install SIERRA from github
...
```
## Annotate peaks
Finally we want to annotate the peaks for genomic features. This is done using the AnnotatePeaksFromGTF function. As part of annotation involves motif matching, we need to load a genome.
This can be installed from Bioconductor. For human download BSgenome.HSapiens.UCSC.hg38 and for mouse download BSgenome.Mmusculus.UCSC.mm10

```
# The following commands will install genome packages onto your local computer. These packages are large and may take a little time to download (depending on network speed)
